...
/**
 * Interface to be implemented by objects than can handle exceptions thrown during execution of a MVC Faces request, in
 * the typical case to error views. Implementors are either registered as beans in the application context or returned
 * from {@link FacesHandler#getExceptionHandlers()}. This interface is similar to the MVC
 * {@link HandlerExceptionResolver} except that it is called when the {@link MvcFacesRequestContext} is still active and
 * that it provides the opportunity to request specific outcomes, such as re-rendering the current request.
 * 
