...
public class FacesHandlerAdapterTests extends TestCase {
	...
	public void testDoHandle() throws Exception {
		...
		RequestControlContextImpl requestContext = new RequestControlContextImpl(externalContext, execution, handler);
		try {
	...
	public void testDefaultModelBinder() throws Exception {
		adapter.afterPropertiesSet();
		assertEquals(DefaultModelBinder.class, ((RequestMappedModelBindingExecutor) adapter.getModelBindingExecutor())
				.getModelBinder().getClass());
		// Quick test that the beanFactory has been injected
		((DefaultModelBinder) ((RequestMappedModelBindingExecutor) adapter.getModelBindingExecutor()).getModelBinder())
				.afterPropertiesSet();
	}
